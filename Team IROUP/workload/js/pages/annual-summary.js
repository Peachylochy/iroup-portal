...
var PageAnnualSummary = (function() {
  ...
  function loadData() {
    ...
    Promise.all([p, cP]).then(function(res) {
      _data = res[0].data;
  ...
  function _buildDemoSummary() {
    var wl = DemoStore.list({ year: _year });
  ...
  function renderFullList(workloads) {
    ...
    var catMap = {};
    (_categories.length ? _categories : DEMO_DATA.categories).forEach(function(c){ catMap[c.id]=c; });
