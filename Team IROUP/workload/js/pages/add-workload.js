...
var PageAddWorkload = (function() {
  ...
  function processAiFile(file) {
    ...
    if (!API.getBaseUrl()) {
      App.toast('AI Import ต้องตั้งค่า Google Apps Script URL ก่อน เพื่อไม่ส่ง API Key จาก browser โดยตรง', 'error');
      return;
    }
    if (!apiKey) {
    ...
    reader.onload = function(e) {
      ...
      var promise = API.aiImport(base64, mediaType, file.name, apiKey);
...
{"title":"...","workDate":"YYYY-MM-DD","category":"CATxx","hours":8,"detail":"...","confidence":"high/medium/low"}`;
  ...
  function loadEditData(id) {
    var p = API.getBaseUrl() ? API.getWorkload(id) : Promise.resolve({ success: true, data: DemoStore.get(id) });
    p.then(function(res) {
      var w = res.data;
      if (!w) return;
  ...
  function submit() {
    ...
    if (_editId) {
      payload.id = _editId;
      p = API.getBaseUrl() ? API.updateWorkload(payload) : Promise.resolve(DemoStore.update(payload));
    } else {
      p = API.getBaseUrl() ? API.createWorkload(payload) : Promise.resolve(DemoStore.create(payload));
    }
