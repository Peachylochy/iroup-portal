...
var PageAllWorkloads = (function() {
  ...
  function render(container, params) {
    ...
    if (params && params.cat) _catFilter = params.cat;
    if (params && params.category) _catFilter = params.category;
  ...
  function renderTable() {
    ...
    el.innerHTML = pageData.map(function(w, i) {
      ...
      return `<tr>
        ...
        <td>
          <div style="font-weight:500;max-width:320px;">${esc(w.title)}</div>
          ${w.detail ? '<div style="font-size:11px;color:var(--text-muted);margin-top:2px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:320px;">' + esc(w.detail.substring(0,80)) + (w.detail.length>80?'...':'') + '</div>' : ''}
        </td>
        <td>
          <span class="cat-pill" style="background:${cat.color||'#64748b'}22;border-color:${cat.color||'#64748b'}44;color:${cat.color||'#94a3b8'};">
            ${esc(cat.icon||'📌')} ${esc(cat.name||w.category)}
          </span>
        </td>
        ...
        <td>
          <div style="display:flex;gap:4px;">
            ...
            <button class="btn btn-ghost btn-icon btn-sm" title="ลบ" onclick="PageAllWorkloads.deleteItem('${esc(w.id)}','${esc(String(w.title || '').replace(/'/g,"\\'"))}')">🗑️</button>
          </div>
        </td>
      </tr>`;
    }).join('');
  ...
  function deleteItem(id, title) {
    ...
    var p = API.getBaseUrl() ? API.deleteWorkload(id) : Promise.resolve(DemoStore.remove(id));
    p.then(function(res) {
