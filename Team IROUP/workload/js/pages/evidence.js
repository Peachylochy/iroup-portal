...
var PageEvidence = (function() {
  ...
  function loadData() {
    ...
    var wP = useApi ? API.listWorkloads({ year: App.getYear() }) : Promise.resolve({ success: true, data: DemoStore.list({ year: App.getYear() }) });
  ...
  function _buildDemoEvidence() {
    var ev = [];
    DemoStore.list({ year: App.getYear() }).forEach(function(w) {
      ...
      return `
        <div class="evidence-card">
          ...
          <div style="flex:1;overflow:hidden;">
            ...
            <div class="evidence-meta">${esc(e.description || '—')}</div>
            <div class="evidence-meta" style="margin-top:2px;">
              📋 ${esc((wl.title||'').substring(0,40))} · ${esc(dateStr)}
            </div>
          </div>
  ...
  function uploadFiles() {
    ...
    var files = _selectedFiles.slice();
    App.toast(API.getBaseUrl() ? 'กำลังอัปโหลดหลักฐานไปยัง Google Drive...' : 'Demo Mode: เพิ่มหลักฐานในหน้านี้เท่านั้น', 'info');

    var uploadChain = Promise.resolve();
    files.forEach(function(f) {
      uploadChain = uploadChain.then(function() {
        if (!API.getBaseUrl()) {
          _evidence.unshift(_buildLocalEvidence(workloadId, f, description, '#', 'demo'));
          return Promise.resolve();
        }
        return _readFileBase64(f).then(function(fileBase64) {
          return API.createEvidence({
            workloadId: workloadId,
            fileName: f.name,
            fileType: f.name.split('.').pop().toLowerCase(),
            fileMediaType: f.type || 'application/octet-stream',
            fileBase64: fileBase64,
            description: description || f.name
          }).then(function(res) {
            _evidence.unshift(_buildLocalEvidence(workloadId, f, description, res.driveUrl || '#', res.driveFileId || '', res.id));
          });
        });
      });
    });
    ...
    uploadChain.then(function() {
      ...
      renderGrid();
      App.toast('อัปโหลดหลักฐานเรียบร้อย ✅', 'success');
    }).catch(function(err) {
      App.toast('อัปโหลดไม่สำเร็จ: ' + err.message, 'error');
    });
  }
  ...
  function _buildLocalEvidence(workloadId, file, description, driveUrl, driveFileId, id) {
    return {
      id: id || ('ev' + Date.now() + Math.random()),
      workloadId: workloadId,
      fileName: file.name,
      fileType: file.name.split('.').pop().toLowerCase(),
      driveFileId: driveFileId || 'demo',
      driveUrl: driveUrl || '#',
  ...
  function addDriveLink() {
    ...
    p.then(function(res) {
      _evidence.unshift(Object.assign({ id: (res && res.id) || ('ev' + Date.now()), uploadedAt: new Date().toISOString().split('T')[0] }, payload));
